"""CPU-safe host preflight for the P4.8f objective-evidence package."""
...
from src.opd.production_b2_calibration_preflight_v1 import _default_gpu_probe
...
EXPECTED_BRANCH = "codex/p4-8g-b2-objective-gpu-revalidation"
# R8 reached the per-step 10 GiB hard floor after the v17 adapter export even
# though the former 5 GiB projection passed at launch.  Reserve a full extra
# GiB for differential evidence, 20 adapter transports, and four sealed
# optimizer checkpoints before any model is loaded.
PROJECTED_INCREMENT_BYTES = 6 * 1024**3


class B2ObjectiveRevalidationPreflightV2Error(RuntimeError):
    """P4.8f is unsafe to start before any model load."""


def _fail(message: str) -> None:
    raise B2ObjectiveRevalidationPreflightV2Error(message)
...
def _default_git_probe() -> dict[str, Any]:
    ...
    return {"branch": branch, "head": head, "clean": clean}
...
def preflight_b2_objective_revalidation_v2(
    ...
    *,
    ...
    if mode not in {"dry-run", "host-preflight", "execute"}:
        _fail("unknown P4.8f preflight mode")
    schedule = package_audit.get("schedule")
    authority = package_audit.get("data_authority")
    contract = package_audit.get("equivalence_contract")
    if not all(
        isinstance(value, Mapping)
        for value in (schedule, authority, contract)
    ):
        _fail("P4.8f verified package audit is incomplete")
    if not (
        ...
        and authority.get("manifest_sha256") == EXPECTED_MANIFEST_SHA256
        and schedule.get("schedule_sha256") == EXPECTED_SCHEDULE_SHA256
    ...
    if not allow_dirty_for_development and not (
        ...
        _fail("P4.8f schema requires a clean P4.8g branch containing package code")
    free = int(
    ...
    if mode in {"host-preflight", "execute"}:
        ...
        if not isinstance(observed, Mapping):
            _fail("P4.8f host requires exactly two idle RTX 3090 GPUs")
        gpu_host = dict(observed)
        raw_gpus = gpu_host.get("gpus")
        gpus = (
            [dict(value) for value in raw_gpus]
        ...
        if not (
            len(gpus) == 2
            and all(
                gpu.get("name") == "NVIDIA GeForce RTX 3090"
                for gpu in gpus
            )
            and all(int(gpu.get("total_mib", 0)) >= 24000 for gpu in gpus)
            and all(
                int(gpu.get("used_mib", 1_000_000)) <= 16 for gpu in gpus
            )
    ...
    return {
        "schema_version": 2,
        "artifact_kind": "p4_8f_b2_objective_revalidation_preflight_v2",
        "status": "ready_waiting_for_gpu_b2_objective_revalidation",
        "mode": mode,
