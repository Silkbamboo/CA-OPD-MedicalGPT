"""On-Policy Distillation core mathematics (docs/METHOD.md §9, §11.4).
...
Sign conventions (docs/METHOD.md §9)
-------------------------------------
...
class OPDBatch:
    ...
    project guarantees "Teacher 不重新生成答案" (docs/METHOD.md §9).
    """
...
def build_opd_batch(
    ...
    max_length
        ...
        silent truncation would change the training signal (docs/REPRODUCIBILITY.md §5:
        "失败要显式报错").
    """
...
def reverse_kl_per_token(student_logprobs: Tensor, teacher_logprobs: Tensor) -> Tensor:
    """``r_t = log pi_S(y_t) - log pi_T(y_t)`` (docs/METHOD.md §9).
...
def teacher_student_advantage(
    ...
    (docs/METHOD.md §9: "不对 Teacher 反向传播").
    """
    if beta <= 0:
...
class DomainKLController:
    """Per-domain reverse-KL EMA and safety scale (docs/METHOD.md §11.4).
...
class PPOStats:
    ...
    def as_metrics(self) -> Dict[str, float]:
        """Map to the frozen metric vocabulary (docs/REPRODUCIBILITY.md §8)."""
        return {
...
def ppo_policy_loss(
    ...
    (docs/METHOD.md §9: "rollout old logprob 不被误替换").
