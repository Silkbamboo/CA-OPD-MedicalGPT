...
def _validate_samples(
    value: Any, *, selected_response_length: int = SELECTED_RESPONSE_LENGTH
) -> list[dict[str, Any]]:
    ...
    for raw in value:
        ...
        rows.append(row)
    if counts != {source: 2 for source in SOURCES}:
        _fail("B2 step source balance is not frozen 2+2")
    return rows
...
def validate_calibration_chain(
    ...
    *,
    ...
    selected_response_length: int = SELECTED_RESPONSE_LENGTH,
    resume_checkpoint_versions: Sequence[int] = (10, 20),
) -> dict[str, Any]:
    selected_response_length = _selected_response_length(selected_response_length)
    ...
    if tuple(resume_checkpoint_versions) not in {
        (10, 20),
        (5, 10, 15, 20),
    }:
        _fail("checkpoint resume policy is not registered")
    expected = initial
    ...
    for index, raw in enumerate(records):
        ...
        should_resume = record["next_policy_version"] in set(
            resume_checkpoint_versions
        )
        if record["checkpoint"]["resume_eligible"] is not should_resume:
            _fail("checkpoint resume policy differs from the registered strategy")
