"""P4.8c bounded-1024, package-bound B2 calibration launcher."""
...
from src.opd.production_b2_calibration_preflight_v3 import (
...
RUN_ID = "qwen3-4b-b2-medical-opd-calibration-p4-8c-1024-seed42"
DEFAULT_PACKAGE = Path(
    "artifacts/outputs/"
    "qwen3-4b-b2-medical-opd-calibration-v3-p4-8c-1024-package"
)
DEFAULT_OUTPUT = Path(
    "artifacts/outputs/"
...
DEFAULT_LAUNCH_SPEC = Path(
    "configs/opd/qwen3_4b_b2_calibration_p4_8c_1024.yaml"
)
DEFAULT_RUN_CARD = Path(
    "configs/run_cards/"
    "qwen3-4b-b2-medical-opd-calibration-p4-8c-1024-seed42.json"
)


class B2CalibrationLauncherV3Error(RuntimeError):
    """P4.8c launcher refusal before unsafe work can start."""


def _fail(message: str) -> None:
    raise B2CalibrationLauncherV3Error(message)
...
def _read_json(path: Path, label: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise B2CalibrationLauncherV3Error(
...
def load_launch_spec_v3(
    ...
    if source.resolve() != canonical or source.is_symlink() or not source.is_file():
        _fail("only the canonical P4.8c launch spec is accepted")
    try:
        value = yaml.safe_load(source.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, yaml.YAMLError) as error:
        raise B2CalibrationLauncherV3Error(
            f"P4.8c launch spec is invalid: {type(error).__name__}"
        ) from error
    if not isinstance(value, dict):
        _fail("P4.8c launch spec is not an object")
    required = {
        "schema_id",
        "schema_version",
        "status",
    ...
    if not required <= set(value):
        _fail("P4.8c launch spec sections are incomplete")
    run = value["run"]
    ...
    if not all(
        ...
        for item in (run, frozen, package, isolation)
    ) or not (
        value["schema_id"] == "ca-opd/p4.8c-b2-calibration-launch/v3"
        ...
        and frozen.get("same_run_length_switch_allowed") is False
        and frozen.get("automatic_further_length_escalation") is False
        ...
        _fail("P4.8c launch contract differs")
    return value


def verify_current_launch_bindings_v3(spec: Mapping[str, Any]) -> dict[str, str]:
    bindings = spec.get("current_bindings")
    if not isinstance(bindings, Mapping) or not bindings:
        _fail("P4.8c executable bindings are absent")
    ...
    for name, descriptor in bindings.items():
        if not (
            ...
            _fail("P4.8c executable binding descriptor is invalid")
        ...
        if not (
            ...
            _fail(f"P4.8c executable binding differs: {name}")
        verified[name] = expected
    card_path = REPO_ROOT / str(spec["run_card"]["path"])
    if card_path.resolve() != (REPO_ROOT / DEFAULT_RUN_CARD).resolve():
        _fail("P4.8c checked-in run-card path differs")
    card = _read_json(card_path, "P4.8c checked-in run card")
    launch_sha = stream_sha256(REPO_ROOT / DEFAULT_LAUNCH_SPEC)
    package = spec["source_package"]
    if not (
        card.get("schema_id") == "ca-opd/p4.8c-b2-calibration-run-card/v3"
        ...
        and card.get("length_escalation_attestation_sha256")
        == package.get("length_escalation_attestation_sha256")
        ...
        and card.get("automatic_further_length_escalation") is False
        and card.get("automatically_start_formal_b2") is False
    ):
        _fail("P4.8c checked-in run-card contract differs")
    return verified
...
def run_package_bound_preflight_v3(
    spec: Mapping[str, Any],
    *,
    mode: str,
    allow_dirty_for_development: bool = False,
) -> tuple[dict[str, Any], dict[str, Any]]:
    verify_current_launch_bindings_v3(spec)
    result = preflight_b2_calibration_v3(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    spec = load_launch_spec_v3(args.launch_spec)
    if not (
        ...
        _fail("loose package/output override differs from the P4.8c launch spec")
    if args.dry_run:
        result, _audit = run_package_bound_preflight_v3(
            spec,
            mode="dry-run",
    ...
    if args.execute_worker:
        ...
        result, audit = run_package_bound_preflight_v3(spec, mode="execute")
        install_worker_signal_handlers()
    ...
    _fail("unknown P4.8c launcher mode")
...
if __name__ == "__main__":
    try:
        ...
        B2CalibrationLauncherV3Error,
        B2CalibrationLauncherV1Error,
        B2CalibrationPreflightV3Error,
        B2CalibrationPackageV3Error,
        B2CalibrationArtifactsV1Error,
    ) as error:
        print(f"P4.8c B2 calibration refused: {error}", file=sys.stderr)
        raise SystemExit(2)
...
__all__ = [
    ...
    "load_launch_spec_v3",
    "main",
    "run_package_bound_preflight_v3",
    "verify_current_launch_bindings_v3",
]
