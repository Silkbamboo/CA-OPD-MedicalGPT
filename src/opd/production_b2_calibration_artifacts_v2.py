...
_P4_8E_RESUME_RELOAD_FIELDS = _RESUME_RELOAD_FIELDS | {
...
_P4_8E_CHECKPOINT_FILES = _CHECKPOINT_FILES | {
    "scheduler_state.pt",
    "sampler_state.json",
    "evidence_index.json",
}
...
class B2CalibrationArtifactStoreV1:
    ...
    def commit_resume_reload(self, raw: Mapping[str, Any]) -> None:
        p4_8e = self.package_binding.get("package_version") in {
            "p4_8e_memory_v5",
            "p4_8e_memory_v6",
            "p4_8e_memory_v7",
        ...
        expected_fields = (
            _P4_8E_RESUME_RELOAD_FIELDS if p4_8e else _RESUME_RELOAD_FIELDS
        )
        ...
        if not (
            ...
            and (
                not p4_8e or value["scheduler_state_restored"] is True
            )
...
def _validate_cleanup(raw: Mapping[str, Any]) -> tuple[dict[str, Any], bool]:
    ...
    valid = bool(
        ...
        and isinstance(value.get("gpu_memory_used_mib"), list)
        and len(value["gpu_memory_used_mib"]) == 2
        and all(
            isinstance(item, int) and 0 <= item <= 16
            for item in value["gpu_memory_used_mib"]
        )
...
def _checkpoint_versions(checkpoint_strategy: str) -> tuple[int, ...]:
    if checkpoint_strategy == "step10_step20_and_final":
        return (10, 20)
    if checkpoint_strategy == "step5_step10_step15_step20_and_final":
        return (5, 10, 15, 20)
    _fail("checkpoint strategy is not registered")
...
def _validate_checkpoints(
    ...
    *,
    checkpoint_strategy: str = "step10_step20_and_final",
) -> dict[str, Any]:
    binding = _read_json(output / "package_binding.json", "package binding")
    p4_8e = binding.get("package_version") in {
        "p4_8e_memory_v5",
        "p4_8e_memory_v6",
        "p4_8e_memory_v7",
        "p4_8f_objective_evidence_v2",
    ...
    expected_checkpoint_files = (
        _P4_8E_CHECKPOINT_FILES if p4_8e else _CHECKPOINT_FILES
    )
    entries = []
    by_version = {
        int(record["next_policy_version"]): record for record in records
    }
    versions = _checkpoint_versions(checkpoint_strategy)
    for version in versions:
        ...
        sampler_state = (
            _read_json(
                directory / "sampler_state.json",
                f"checkpoint v{version} sampler state",
            )
            if p4_8e
            else None
        )
        ...
        if not (
            ...
            and set(files) == expected_checkpoint_files
            and actual
            == expected_checkpoint_files
            | {
            ...
            | ({"complete_marker.json"} if p4_8e else set())
            ...
            and (
                not p4_8e
                or (
                    manifest.get("scheduler_step") == version
                    and manifest.get("data_cursor") == version * 4
                    and isinstance(manifest.get("manifest_sha256"), str)
                    and isinstance(manifest.get("schedule_sha256"), str)
                    and manifest.get("evidence_index_sha256")
                    == _stream_sha256(directory / "evidence_index.json")
                    and sampler_state.get("policy_version") == version
                    ...
                    and _read_json(
                        directory / "complete_marker.json",
                        f"checkpoint v{version} complete marker",
                    ).get("checkpoint_manifest_sha256")
                    == _stream_sha256(manifest_path)
                )
            )
        ):
...
def _validate_resume_reload(output: Path, expected_sha: str) -> dict[str, Any]:
    ...
    if not (
        ...
        and (not p4_8e or value.get("scheduler_state_restored") is True)
...
def validate_p4_8e_revalidation_gates(
    output_dir: str | Path,
) -> dict[str, Any]:
    """Recompute the P4.8e differential/canary execution gates from disk.

    This validator intentionally runs before the generic finalizer can write a
    success summary.  It therefore closes the interruption window between the
    20-step worker return and the gate-status handoff.
    """

    output = Path(output_dir).resolve()
    binding = _read_json(output / "package_binding.json", "package binding")
    if binding.get("package_version") not in {
        "p4_8e_memory_v5",
        "p4_8e_memory_v6",
        "p4_8e_memory_v7",
    }:
        _fail("P4.8e gate validation requires the P4.8e package binding")
    gate = _read_json(
        output / "p4_8e_gate_status.json", "P4.8e GPU gate status"
    )
    config = _read_json(output / "config.yaml", "config")
    run_id = str(config.get("run_id", ""))
    report_path = output / "gpu_math_equivalence.json"
    report = _read_json(report_path, "P4.8e GPU differential report")
    canary_path = output / "memory_canary.json"
    canary = _read_json(canary_path, "P4.8e max-shape canary")
    drift_path = output / "memory_six_step_drift.json"
    drift = _read_json(drift_path, "P4.8e six-step memory drift")
    tolerance = {
        "scalar_atol": 1e-6,
        "scalar_rtol": 1e-6,
        "gradient_atol": 2e-6,
    ...
    cleanup = report.get("gpu_cleanup")
    canary_cleanup = canary.get("cleanup_before_formal_fresh_v0")
    if not (
        gate.get("schema_version") == 1
        and gate.get("artifact_kind") == "p4_8e_gpu_gate_status_v1"
        and gate.get("run_id") == run_id
        and gate.get("gpu_math_differential_passed") is True
        and gate.get("gpu_math_report_sha256") == _stream_sha256(report_path)
        and gate.get("max_shape_canary_passed") is True
        and gate.get("max_shape_canary_sha256") == _stream_sha256(canary_path)
        and gate.get("six_step_memory_drift_sha256")
        == _stream_sha256(drift_path)
        and gate.get("steps_completed") == B2_CALIBRATION_STEPS
        and gate.get("B2_formal_authorized") is False
        and report.get("passed") is True
        and report.get("tolerance") == tolerance
        and report.get("strict_counts_equal") is True
        and report.get("runtime_loading") == "serial"
        and report.get("rollout_tokens_persisted") is False
        and report.get("throwaway_adapter_reused") is False
        and report.get("gpu_cleanup_after_gate") is True
        and isinstance(cleanup, Mapping)
        and cleanup.get("memory_allocated_bytes") == [0, 0]
        and cleanup.get("memory_reserved_bytes") == [0, 0]
        and cleanup.get("runtime_references_released") is True
    ):
        _fail("P4.8e GPU differential status/SHA/cleanup gate differs")
    if not (
        canary.get("artifact_kind") == "b2_memory_canary_v1"
        and canary.get("passed") is True
        and canary.get("risk_schedule_step") == 4
        and canary.get("selected_response_length") == 1024
        and canary.get("real_rollout_executed") is True
        and canary.get("fixture_valid_completion_tokens") == 1024
        and canary.get("fixture_source")
        == "prompt_only_legal_synthetic_token_shape"
        and canary.get("optimizer_steps_executed_in_throwaway_session") == 1
        and canary.get("scheduler_steps_executed_in_throwaway_session") == 1
        and canary.get("sampler_refresh_count_in_throwaway_session") == 1
        and canary.get("backbone_backward_calls") == 4
        and int(canary.get("lm_head_chunk_count", 0)) >= 4
        and canary.get("retain_graph_calls") == 0
        and canary.get("teacher_gradient_tensor_count") == 0
        and canary.get("base_gradient_tensor_count") == 0
        and canary.get("p_old_detached") is True
        and canary.get("initial_registry_count")
        == canary.get("final_registry_count")
        and canary.get("initial_model_count") == canary.get("final_model_count")
        and canary.get("raw_prompt_persisted") is False
        and canary.get("response_tokens_persisted") is False
        and canary.get("label_access_count") == 0
        and canary.get("controller_access_count") == 0
        and canary.get("final_access_count") == 0
        and isinstance(canary.get("minimum_free_bytes_by_gpu"), list)
        and len(canary["minimum_free_bytes_by_gpu"]) == 2
        and all(
            int(value) >= 1024**3
            for value in canary["minimum_free_bytes_by_gpu"]
        )
        and isinstance(canary_cleanup, Mapping)
        and canary_cleanup.get("memory_allocated_bytes") == [0, 0]
        and canary_cleanup.get("memory_reserved_bytes") == [0, 0]
        and canary_cleanup.get("runtime_references_released") is True
        and drift.get("passed") is True
        and drift.get("steps") == [1, 2, 3, 4, 5, 6]
    ):
        _fail("P4.8e max-shape canary or six-step gate differs")
    ...
    try:
        ...
        memory_audit = validate_memory_run_artifacts(output, records)
    except (OSError, RuntimeError, ValueError) as error:
        raise B2CalibrationArtifactsV1Error(
            "P4.8e memory disk evidence differs"
        ) from error
    for step in range(1, B2_CALIBRATION_STEPS + 1):
        audit = _read_json(
            output / "memory_step_audits" / f"step_{step:02d}.json",
            f"P4.8e memory step {step} audit",
        )
        if not (
            audit.get("backbone_backward_calls_per_prompt") == 1
            and int(audit.get("lm_head_chunk_count", 0)) >= 4
            and audit.get("retain_graph_calls") == 0
            and audit.get("scheduler_step_count") == 1
            and audit.get("optimizer_step_count") == 1
            and audit.get("sampler_refresh_count") == 1
        ):
            _fail(f"P4.8e memory step {step} execution topology differs")
    return {
        "schema_version": 1,
        "artifact_kind": "p4_8e_revalidation_disk_gate_audit_v1",
        "passed": True,
        "gpu_math_report_sha256": _stream_sha256(report_path),
        "max_shape_canary_sha256": _stream_sha256(canary_path),
        "six_step_memory_drift_sha256": _stream_sha256(drift_path),
        "step_audit_count": B2_CALIBRATION_STEPS,
        "memory_telemetry_sha256": memory_audit["telemetry_sha256"],
        "B2_formal_authorized": False,
    }


def validate_p4_8f_revalidation_gates(
    output_dir: str | Path,
) -> dict[str, Any]:
    """Recompute P4.8f two-layer, canary, drift, and topology gates."""

    output = Path(output_dir).resolve()
    binding = _read_json(output / "package_binding.json", "package binding")
    if binding.get("package_version") != "p4_8f_objective_evidence_v2":
        _fail("P4.8f gate validation requires the P4.8f package binding")
    gate = _read_json(
        output / "p4_8f_gate_status.json", "P4.8f GPU gate status"
    )
    config = _read_json(output / "config.yaml", "config")
    run_id = str(config.get("run_id", ""))
    report_path = output / "gpu_math_equivalence.json"
    report = _read_json(report_path, "P4.8f GPU differential report")
    diagnostic_path = output / "gpu_math_diagnostic_comparison.json"
    diagnostic = _read_json(
        diagnostic_path, "P4.8f differential diagnostic comparison"
    )
    canary_path = output / "memory_canary.json"
    canary = _read_json(canary_path, "P4.8f max-shape canary")
    drift_path = output / "memory_six_step_drift.json"
    drift = _read_json(drift_path, "P4.8f six-step memory drift")
    cleanup = report.get("gpu_cleanup")
    canary_cleanup = canary.get("cleanup_before_formal_fresh_v0")
    final_component = diagnostic.get("components", {}).get("final_decision", {})
    if not (
        gate.get("schema_version") == 2
        and gate.get("artifact_kind") == "p4_8f_gpu_gate_status_v2"
        and gate.get("run_id") == run_id
        and isinstance(gate.get("legacy_strict_1e6_pass"), bool)
        and gate.get("algebraic_semantic_equivalence_pass") is True
        and gate.get("gpu_bf16_operational_equivalence_pass") is True
        and gate.get("gpu_math_report_sha256") == _stream_sha256(report_path)
        and gate.get("gpu_math_diagnostic_sha256")
        == _stream_sha256(diagnostic_path)
        and gate.get("max_shape_canary_passed") is True
        and gate.get("max_shape_canary_sha256") == _stream_sha256(canary_path)
        and gate.get("six_step_memory_drift_sha256")
        == _stream_sha256(drift_path)
        and gate.get("steps_completed") == B2_CALIBRATION_STEPS
        and gate.get("B2_formal_authorized") is False
        and isinstance(report.get("legacy_strict_1e6_pass"), bool)
        and report.get("algebraic_semantic_equivalence_pass") is True
        and report.get("gpu_bf16_operational_equivalence_pass") is True
        and report.get("can_enter_max_shape_canary") is True
        and diagnostic.get("status") == "passed"
        and diagnostic.get("algebraic_semantic_equivalence_pass") is True
        and diagnostic.get("gpu_bf16_operational_equivalence_pass") is True
        and final_component.get("status") == "complete"
        and report.get("runtime_loading") == "serial"
        and report.get("rollout_tokens_persisted") is False
        and report.get("throwaway_adapter_reused") is False
        and report.get("gpu_cleanup_after_gate") is True
        ...
        _fail("P4.8f differential status/SHA/cleanup gate differs")
    if not (
        canary.get("artifact_kind") == "b2_memory_canary_v1"
        and canary.get("passed") is True
        and canary.get("risk_schedule_step") == 4
        ...
        _fail("P4.8f max-shape canary or six-step gate differs")
    records = _read_metrics(output / "metrics.jsonl")
    try:
        from src.opd.production_b2_memory_execution_v1 import (
            validate_memory_run_artifacts,
        ...
        raise B2CalibrationArtifactsV1Error(
            "P4.8f memory disk evidence differs"
        ) from error
    for step in range(1, B2_CALIBRATION_STEPS + 1):
        audit = _read_json(
            output / "memory_step_audits" / f"step_{step:02d}.json",
            f"P4.8f memory step {step} audit",
        )
        if not (
            audit.get("backbone_backward_calls_per_prompt") == 1
            and int(audit.get("lm_head_chunk_count", 0)) >= 4
            ...
            _fail(f"P4.8f memory step {step} execution topology differs")
    return {
        "schema_version": 2,
        "artifact_kind": "p4_8f_revalidation_disk_gate_audit_v2",
        "passed": True,
        "legacy_strict_1e6_pass": gate["legacy_strict_1e6_pass"],
        "algebraic_semantic_equivalence_pass": True,
        "gpu_bf16_operational_equivalence_pass": True,
        "gpu_math_report_sha256": _stream_sha256(report_path),
        "gpu_math_diagnostic_sha256": _stream_sha256(diagnostic_path),
        "max_shape_canary_sha256": _stream_sha256(canary_path),
        "six_step_memory_drift_sha256": _stream_sha256(drift_path),
        "step_audit_count": B2_CALIBRATION_STEPS,
        "memory_telemetry_sha256": memory_audit["telemetry_sha256"],
...
def finalize_calibration_run(
    ...
    *,
    ...
    config = _read_json(output / "config.yaml", "config")
    binding = _read_json(output / "package_binding.json", "package binding")
    p4_8e_gate_audit = None
    p4_8e_gate_error = None
    p4_8f_gate_audit = None
    p4_8f_gate_error = None
    if binding.get("package_version") == "p4_8f_objective_evidence_v2":
        try:
            p4_8f_gate_audit = validate_p4_8f_revalidation_gates(output)
        except B2CalibrationArtifactsV1Error as error:
            p4_8f_gate_error = type(error).__name__
    elif binding.get("package_version") in {
        "p4_8e_memory_v5",
        "p4_8e_memory_v6",
        "p4_8e_memory_v7",
    }:
        try:
            p4_8e_gate_audit = validate_p4_8e_revalidation_gates(output)
        except B2CalibrationArtifactsV1Error as error:
            # A missing/failed success gate is terminal for success, but must
            # not prevent durable sealing of an already recorded worker
            # failure or interruption.
            p4_8e_gate_error = type(error).__name__
    ...
    memory_execution_audit: dict[str, Any] | None = None
    checkpoint_index: dict[str, Any] = {
        "schema_version": 1,
        "artifact_kind": "b2_calibration_checkpoint_index_v1",
        "run_id": run_id,
        "checkpoint_strategy": str(
            config.get("checkpoint_strategy", "step10_step20_and_final")
        ),
        "checkpoints": [],
    ...
    if worker_classification is not None:
    ...
    elif p4_8e_gate_error is not None:
        status = "failed_artifact_integrity"
        reason = (
            "p4_8e_gpu_gate_revalidation:"
            f"{p4_8e_gate_error}"
        )
    elif p4_8f_gate_error is not None:
        status = "failed_artifact_integrity"
        reason = (
            "p4_8f_gpu_gate_revalidation:"
            f"{p4_8f_gate_error}"
        )
        ...
        try:
            chain = validate_calibration_chain(
                ...
                resume_checkpoint_versions=_checkpoint_versions(
                    str(
                        config.get(
        ...
        if chain is not None:
            try:
                checkpoint_index = _validate_checkpoints(
                    output,
                    records,
                    checkpoint_strategy=str(
                        config.get(
                ...
                if (
                    config.get("checkpoint_strategy")
                    ...
                    try:
                        ...
                        memory_execution_audit = validate_memory_run_artifacts(
                            output, records
                        )
                    except (MemoryError, OSError, RuntimeError, ValueError):
                        status = "failed_b2_calibration_memory_evidence"
                        reason = "memory_canary_drift_or_lifecycle_artifact_invalid"
                        chain = None
                if chain is not None:
                    try:
                ...
                if chain is not None:
                    try:
                        _validate_final_reload(
                            output, chain["final_adapter_sha256"]
                        )
                    except B2CalibrationArtifactsV1Error:
    ...
    summary = {
        ...
        "resources": resources,
        "memory_execution": memory_execution_audit,
        "p4_8e_revalidation": (
            p4_8e_gate_audit
            if p4_8e_gate_audit is not None
            else (
                {
                    "passed": False,
                    "error_type": p4_8e_gate_error,
                    "B2_formal_authorized": False,
                }
                if binding.get("package_version")
                in {
                    "p4_8e_memory_v5",
                    "p4_8e_memory_v6",
                    "p4_8e_memory_v7",
        ...
        "p4_8f_revalidation": (
            p4_8f_gate_audit
            if p4_8f_gate_audit is not None
            else (
                {
                    "passed": False,
                    "error_type": p4_8f_gate_error,
                    "B2_formal_authorized": False,
                }
                if binding.get("package_version")
                == "p4_8f_objective_evidence_v2"
...
def recompute_calibration_readiness(output_dir: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        binding = _read_json(output / "package_binding.json", "package binding")
        if binding.get("package_version") == "p4_8f_objective_evidence_v2":
            validate_p4_8f_revalidation_gates(output)
        elif binding.get("package_version") in {
            "p4_8e_memory_v5",
            "p4_8e_memory_v6",
            "p4_8e_memory_v7",
        }:
            validate_p4_8e_revalidation_gates(output)
        selected_response_length = _selected_length(config, binding)
        ...
        chain = validate_calibration_chain(
            ...
            selected_response_length=selected_response_length,
            resume_checkpoint_versions=_checkpoint_versions(
                str(config.get("checkpoint_strategy", "step10_step20_and_final"))
            ),
        )
        ...
        if (
            ...
            validate_memory_run_artifacts(output, records)
...
__all__ = [
    ...
    "validate_p4_8e_revalidation_gates",
    "validate_p4_8f_revalidation_gates",
]
