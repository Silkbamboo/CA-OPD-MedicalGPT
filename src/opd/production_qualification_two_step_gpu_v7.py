...
def _b2_optimizer_step_limit(config: Mapping[str, Any]) -> int:
    """Keep calibration at 20; widen only for the exact P5 formal envelope."""

    if "formal_b2" not in config:
        return 20
    p8 = config.get("p8_formal_b2")
    if p8 is not None:
        if not (
            isinstance(p8, Mapping)
            and p8.get("package_version") == "p8_single_variable_b2_v1"
            and p8.get("single_training_semantic_variable") == "medical_source_mix"
            and p8.get("source_batch")
            == {"medical_opd_o1": 3, "medical_opd_cmb": 1}
            and p8.get("frozen_max_step") == 300
            and p8.get("stage1_stop_step") == 120
            and p8.get("group_size") == 1
            and float(p8.get("learning_rate", -1.0)) == 1e-5
            and float(p8.get("per_prompt_gradient_clip_norm", -1.0)) == 0.25
            and p8.get("response_length") == 1024
        ):
            raise ProductionTwoStepQualificationV6Error(
                "P8 formal single-variable step envelope differs"
            )
        return 300
    from src.opd.production_b2_formal_v1 import formal_step_limit

    return formal_step_limit(config)
...
class ProductionTwoStepSessionV6:
    ...
    def _memory_phase_observer(
        ...
        *,
        ...
        """Optional package-gated memory observer; legacy routes are no-ops."""

        del marker, phase, step, sequence_shape, token_shape

    def _initialize_models(self) -> None:
        torch = self.torch
        self._memory_phase_observer("before", "tokenizer_and_base_load")
        ...
        self._memory_phase_observer("after", "tokenizer_and_base_load")
        optimizer = self.optimizer_config
        self._memory_phase_observer("before", "fresh_student_on_gpu0")
        ...
        self._memory_phase_observer("after", "fresh_student_on_gpu0")
        ...
        self.scheduler = build_constant_lr_scheduler(torch, self.optimizer)

        v0_path = self.scratch / "v0"
        self.student_model.save_pretrained(v0_path, safe_serialization=True)
        self._memory_phase_observer("before", "fresh_sampler_on_gpu1")
        ...
        self._memory_phase_observer("after", "fresh_sampler_on_gpu1")
    ...
    def _initialize_b2_checkpoint(
        ...
        *,
        ...
        if not (
            self._trainable_identity_equal(trainer, saved)
    ...
    def _trainable_identity_equal(
        self, trainer: Mapping[str, Any], runtime: Mapping[str, Any]
    ) -> bool:
        """Compare one trainable PEFT load with its frozen runtime authority.

        PEFT changes only ``inference_mode`` from true in the saved/runtime
        adapter config to false when the same adapter is loaded with
        ``is_trainable=True``.  All tensor, structural config and revision
        fields remain identity-critical.
        """

        trainer_config = trainer.get("canonical_config")
        runtime_config = runtime.get("canonical_config")
        if not (
            isinstance(trainer_config, Mapping)
            and isinstance(runtime_config, Mapping)
            and trainer_config.get("inference_mode") is False
            and runtime_config.get("inference_mode") is True
        ):
            return False
        expected_trainer_config = dict(runtime_config)
        expected_trainer_config["inference_mode"] = False
        return bool(
            self._tensor_identity_equal(trainer, runtime)
            and dict(trainer_config) == expected_trainer_config
            and trainer.get("canonical_config_sha256")
            == _canonical_sha(expected_trainer_config)
            and runtime.get("canonical_config_sha256")
            == _canonical_sha(dict(runtime_config))
            and trainer.get("base_revision") == runtime.get("base_revision")
            and trainer.get("tokenizer_revision")
    ...
    def _score_step_teacher_rows(
        ...
        *,
        ...
        """Score the default frozen Medical Teacher on the Student tokens.

        Formal multi-teacher methods override this narrow hook.  Keeping the
        historical Medical-only body here makes the B2 execution path byte-for-
        byte equivalent apart from the call boundary.
        """

        del old_actor, old_mask
    ...
    def _backward_corrected_rows(
        ...
        *,
        ...
        """Legacy row-wise accumulation hook retained for historical routes."""
        ...
        for row_index, row in enumerate(rows):
            ...
            row_result = self.decoupled_corrected_objective(
                ...
                advantage_scale=(
                    None
                    if getattr(self, "_current_advantage_scale", None) is None
                    else self._current_advantage_scale[
                        row_index : row_index + 1, :length
                    ]
                ),
            )
    ...
    def run_corrected_step(
        ...
        teacher, teacher_mask = self._score_step_teacher_rows(
            rows,
        ...
        pre_update_hook = getattr(self, "_validate_pre_update_ratio_contract_v2", None)
        if callable(pre_update_hook):
            pre_update_hook(
                step_index=step_index,
                rows=rows,
                bundle=bundle,
        ...
        legacy_correction_gate_passed = bool(
        ...
        v2_correction_hook = getattr(
            self, "_preupdate_correction_gate_v2_passed", None
        )
        correction_gate_passed = (
            bool(v2_correction_hook())
            if callable(v2_correction_hook)
            else legacy_correction_gate_passed
        )
        if not correction_gate_passed:
            if not callable(v2_correction_hook):
                rejection_hook = getattr(
                    self, "_record_correction_gate_rejection_v2", None
                )
        ...
        self._memory_phase_observer(
            "before", "gradient_clip", step=step_index + 1
        )
        ...
        self._memory_phase_observer("after", "gradient_clip")
        gradients = {
            name: (
        ...
        transaction_hook = getattr(self, "_prepare_candidate_transaction_v2", None)
        if callable(transaction_hook):
            transaction_hook(
                step_index=step_index,
                rows=rows,
                bundle=bundle,
        ...
        self._before_optimizer_step(step_index=step_index)
        self.optimizer.step()
        from src.opd.production_b2_memory_checkpoint_v2 import (
        ...
        step_constant_lr_scheduler(
            self.scheduler,
            self.optimizer,
            expected_learning_rate=float(self.optimizer_config["learning_rate"]),
        )
        self._scheduler_step_count += 1
        self._after_optimizer_step(step_index=step_index)
        ...
        frozen_parameter_versions_unchanged = all(
        ...
        legacy_candidate_gate_evidence = {
            ...
            "alignment": alignment,
            "alignment_positive": alignment > 0,
            "optimizer_update_audit_passed": update_audit.hard_gate_passed,
            "optimizer_update_failure_reasons": list(update_audit.failure_reasons),
            "teacher_gradient_tensor_count": teacher_gradient_count,
            "teacher_gradient_free": teacher_gradient_count == 0,
            "base_gradient_tensor_count": base_gradient_count,
            "base_gradient_free": base_gradient_count == 0,
        ...
        legacy_candidate_gate_passed = bool(
            all(
                legacy_candidate_gate_evidence[key]
                for key in (
                    "objective_improved",
                    "loss_decreased",
                    "alignment_positive",
        ...
        legacy_candidate_gate_evidence["passed"] = legacy_candidate_gate_passed
        candidate_hook = getattr(self, "_validate_candidate_update_v2", None)
        if callable(candidate_hook):
            candidate_hook(
                ...
                gradient_norm_before_clip=gradient_before_clip,
                telemetry=telemetry,
                legacy_candidate_gate_passed=legacy_candidate_gate_passed,
                legacy_candidate_gate_evidence=legacy_candidate_gate_evidence,
            )
        elif not legacy_candidate_gate_passed:
            raise ProductionTwoStepQualificationV6Error(
        ...
        try:
            ...
            abort_hook = getattr(self, "_abort_candidate_transaction_v2", None)
            if callable(abort_hook):
                abort_hook(reason="candidate_export_failed")
            raise
        checkpoint_seconds = time.perf_counter() - checkpoint_started
        if self.route in {"b2", "b2_calibration"}:
            trajectory_objectives = [
                float(value)
                for value in after_result.reduction.trajectory_means.detach()
                .float()
                .cpu()
                .tolist()
            ]
            per_source_objective = {
                source: sum(
                    ...
                    for value, role in zip(
                        trajectory_objectives, source_roles, strict=True
                    )
                    if role == source
                )
                / sum(role == source for role in source_roles)
            ...
            self._last_b2_step_private = {
                ...
                "per_source_objective": per_source_objective,
                "loss": loss_after,
        ...
        if not getattr(self, "_transactional_commit_v2_enabled", False):
    ...
    def _source_counts_for_step_artifact(
        ...
        for row in rows:
            ...
            if not isinstance(role, str) or not role:
                raise ProductionTwoStepQualificationV6Error(
                    "step artifact source role is absent"
                )
            counts[role] = counts.get(role, 0) + 1
        if sum(counts.values()) != 4:
            raise ProductionTwoStepQualificationV6Error(
                "step artifact source count differs from four"
            )
        return counts
    ...
    def run_b2_calibration_step(
        ...
        *,
        ...
        optimizer_step_limit = _b2_optimizer_step_limit(self.config)
        if not (
            self.route in {"b2", "b2_calibration"}
            and isinstance(step_index, int)
            and not isinstance(step_index, bool)
            and 0 <= step_index < optimizer_step_limit
            and from_version
        ...
        try:
            ...
            abort_hook = getattr(self, "_abort_candidate_transaction_v2", None)
            if callable(abort_hook):
                abort_hook(reason="candidate_execution_failed")
        ...
        try:
            ...
            abort_hook = getattr(self, "_abort_candidate_transaction_v2", None)
            if callable(abort_hook):
                abort_hook(reason="sampler_refresh_failed")
            raise
        ...
        commit_hook = getattr(self, "_commit_candidate_transaction_v2", None)
        if callable(commit_hook):
            commit_hook(step_index=step_index, refresh=refresh)
        ...
        artifact = {
            ...
            "prompt_count": 4,
            "source_counts": self._source_counts_for_step_artifact(generated),
            "raw_prompt_persisted": False,
            "response_tokens_persisted": False,
        ...
        *,
        ...
        evidence = {
            ...
            "checkpoint": {
                ...
                "complete": True,
                "resume_eligible": version + 1 in (
                    {5, 10, 15, 20}
                    if "memory_execution" in self.config
                    else {10, 20}
                ),
            },
            "isolation": dict(self.config["isolation"]),
        }
        expected_source_counts_builder = getattr(
            self, "_expected_step_record_source_counts_v4", None
        )
        expected_source_counts = (
            expected_source_counts_builder()
            if callable(expected_source_counts_builder)
            else None
        )
        return build_b2_step_record_v1(
            evidence,
            selected_response_length=max_new_tokens,
    ...
    def save_b2_resume_checkpoint_v1(
        ...
        *,
        ...
        """Seal package-declared complete adapter versions for exact resume."""

        p4e = self.config.get("package_version") in {
            "p4_8e_memory_v5",
            "p4_8e_memory_v6",
            "p4_8e_memory_v7",
        ...
        eligible_versions = (
            {5, 10, 15, 20} if "memory_execution" in self.config else {10, 20}
        )
        if not (
            self.route == "b2_calibration"
            and logical_version in eligible_versions
        ...
        if p4e:
            if (
                ...
                or self._scheduler_step_count != logical_version
            ):
                raise ProductionTwoStepQualificationV6Error(
                    "resume scheduler state/version differs"
                )
            self._atomic_torch_save(
                checkpoint / "scheduler_state.pt", self.scheduler.state_dict()
            )
        self._atomic_torch_save(
        ...
        schedule_sha = None
        if p4e:
            data = _mapping(self.config.get("data"), "resume data identity")
            manifest_sha = _digest(
                data.get("prompt_manifest_sha256"), "resume manifest"
            )
            schedule_sha = _digest(
                data.get("schedule_sha256"), "resume schedule"
            )
            sampler_runtime = _mapping(
                self.current_sampler_runtime, "resume sampler runtime"
            )
            _atomic_json(
                checkpoint / "sampler_state.json",
                {
                    "policy_version": logical_version,
                    "runtime_adapter_sha256": _digest(
                        sampler_runtime.get("aggregate_tensor_sha256"),
                        "resume sampler runtime",
                    ),
                    "active_adapter": sampler_runtime.get("active_adapter"),
                    "registry_count": _mapping(
                        sampler_runtime.get("registry_snapshot"),
                        "resume sampler registry",
                    ).get("adapter_count"),
                },
            )
            evidence_files: list[dict[str, Any]] = []
            for evidence_root in (
                self.output / "b2_steps",
                self.output / "memory_step_audits",
                self.output / "memory_telemetry" / "markers",
            ):
                    ...
                    for evidence_path in sorted(evidence_root.glob("*.json")):
                        if (
                            ...
                            evidence_files.append(
                                {
                                    "path": str(
                                        evidence_path.relative_to(self.output)
                                    ),
            ...
            _atomic_json(
                checkpoint / "evidence_index.json",
                {
                    "schema_version": 1,
                    "artifact_kind": "b2_checkpoint_evidence_index_v1",
                    "run_id": self.config["run"]["run_id"],
            ...
            state.update(
                {
                    "scheduler_step": logical_version,
        ...
        if p4e:
            file_names.extend(
                ["scheduler_state.pt", "sampler_state.json", "evidence_index.json"]
            )
        files = {
            name: {
                "sha256": _sha_file(checkpoint / name),
            ...
            manifest.update(
                {
                    ...
                    "manifest_sha256": manifest_sha,
                    "schedule_sha256": schedule_sha,
                    "evidence_index_sha256": _sha_file(
                        checkpoint / "evidence_index.json"
                    ),
                }
            )
        ...
        if p4e:
            _atomic_json(
                checkpoint / "complete_marker.json",
                {
                    "schema_version": 1,
                    "artifact_kind": "b2_resume_checkpoint_complete_marker_v1",
                    "run_id": self.config["run"]["run_id"],
                    "logical_version": logical_version,
    ...
    def reload_b2_resume_checkpoint_v1(
        ...
        *,
        ...
        try:
            ...
            complete_marker = (
                json.loads(
                    (checkpoint / "complete_marker.json").read_text(
                        encoding="utf-8"
                    )
                )
                if p4e
            ...
            sampler_state = (
                json.loads(
                    (checkpoint / "sampler_state.json").read_text(
                        encoding="utf-8"
                    )
                )
                if p4e
        ...
        if not (
            ...
            and (
                ...
                or (
                    data is not None
                    and manifest.get("scheduler_step") == 10
                    and manifest.get("data_cursor") == 40
                    and manifest.get("manifest_sha256")
                    == expected_state["manifest_sha256"]
                    and manifest.get("schedule_sha256")
                    == expected_state["schedule_sha256"]
                    and manifest.get("evidence_index_sha256")
                    == _sha_file(checkpoint / "evidence_index.json")
                    and complete_marker
                    == {
                        "schema_version": 1,
                        "artifact_kind": (
                            "b2_resume_checkpoint_complete_marker_v1"
                        ),
                        "run_id": self.config["run"]["run_id"],
                        "logical_version": 10,
                    ...
                    and sampler_state.get("policy_version") == 10
                    and sampler_state.get("active_adapter") == PRODUCTION_SLOT
        ...
        if (
            manifest.get("adapter_sha256") != expected_sha
            or (
                p4e
                and sampler_state.get("runtime_adapter_sha256") != expected_sha
            )
        ):
            raise ProductionTwoStepQualificationV6Error(
        ...
        if p4e:
            scheduler_state = self.torch.load(
                checkpoint / "scheduler_state.pt",
                map_location="cpu",
                weights_only=True,
            )
            self.scheduler.load_state_dict(scheduler_state)
            if int(self.scheduler.state_dict().get("last_epoch", -1)) != 10:
                raise ProductionTwoStepQualificationV6Error(
                    "resume scheduler step differs"
                )
        sampler_base = self.AutoModelForCausalLM.from_pretrained(
            self.model_path,
            local_files_only=True,
        ...
        if not (
            self._trainable_identity_equal(trainer, target)
        ...
        if p4e:
            result["scheduler_state_restored"] = True
        return result
    ...
    def final_checkpoint_reload_identity_v1(self) -> Mapping[str, Any]:
        ...
        if p4e:
            manifest_path = checkpoint / "checkpoint_manifest.json"
            complete_path = checkpoint / "complete_marker.json"
            try:
                manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
                ...
                raise ProductionTwoStepQualificationV6Error(
                    f"final checkpoint metadata invalid: {type(error).__name__}"
                ) from error
            if not (
                manifest.get("logical_version") == 20
                and manifest.get("adapter_sha256") == target_sha
                and manifest.get("optimizer_step") == 20
                and manifest.get("scheduler_step") == 20
                and manifest.get("data_cursor") == 80
                and manifest.get("complete") is True
                and manifest.get("resume_eligible") is True
                and calibration_state.get("optimizer_step") == 20
                and calibration_state.get("scheduler_step") == 20
                and calibration_state.get("policy_version") == 20
                and calibration_state.get("data_cursor") == 80
                and sampler_state.get("policy_version") == 20
                and sampler_state.get("runtime_adapter_sha256") == target_sha
                and sampler_state.get("active_adapter") == PRODUCTION_SLOT
                and sampler_state.get("registry_count") == 1
                and complete_marker.get("logical_version") == 20
                and complete_marker.get("checkpoint_manifest_sha256")
                == _sha_file(manifest_path)
                and complete_marker.get("complete") is True
                and complete_marker.get("resume_eligible") is True
            ):
                raise ProductionTwoStepQualificationV6Error(
                    "final checkpoint complete/recovery marker differs"
                )
        self._release(
            self.torch,
            self.teacher_model,
        ...
        if p4e:
            ...
            if len(fresh_parameters) != 504:
                raise ProductionTwoStepQualificationV6Error(
                    "final checkpoint trainable recovery scope differs"
                )
            optimizer_config = self.optimizer_config
            recovery_optimizer = self.torch.optim.AdamW(
                fresh_parameters,
            ...
            recovery_optimizer.load_state_dict(
                self.torch.load(
                    checkpoint / "optimizer_state.pt",
                    map_location="cpu",
                    weights_only=True,
            ...
            recovery_scheduler = build_constant_lr_scheduler(
                self.torch, recovery_optimizer
            )
            recovery_scheduler.load_state_dict(
                self.torch.load(
                    checkpoint / "scheduler_state.pt",
                    map_location="cpu",
                    weights_only=True,
            ...
            if not (
                int(recovery_scheduler.state_dict().get("last_epoch", -1)) == 20
                and isinstance(rng, Mapping)
                and isinstance(rng.get("cuda"), list)
                and len(rng["cuda"]) == 2
            ):
                raise ProductionTwoStepQualificationV6Error(
                    "final checkpoint optimizer/scheduler/RNG recovery differs"
                )
            self.torch.set_rng_state(rng["cpu"])
            self.torch.cuda.set_rng_state_all(rng["cuda"])
            _atomic_json(
                self.output / "final_checkpoint_recovery_v20.json",
                {
                    "schema_version": 1,
                    "artifact_kind": "b2_final_checkpoint_recovery_v20_v1",
                    "run_id": self.config["run"]["run_id"],
                    "logical_version": 20,
                    "adapter_sha256": fresh_identity[
                        "aggregate_tensor_sha256"
                    ],
                    "optimizer_state_restored": True,
            ...
            del recovery_optimizer, recovery_scheduler, rng, fresh_parameters
        self._release(self.torch, fresh)
        if not (
            self._identity_equal(target, runtime)
            and self._trainable_identity_equal(fresh_identity, target)
...
def _b2_runtime_config(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if schema not in {
        ...
        ("ca-opd/b2-medical-opd-calibration/v4", 4),
    }:
    ...
    if schema == ("ca-opd/b2-medical-opd-calibration/v4", 4):
        from src.opd.production_b2_memory_execution_v1 import (
            validate_memory_execution_contract,
        )

        projected["memory_execution"] = validate_memory_execution_contract(
            _mapping(config.get("memory_execution"), "B2 memory execution")
        )
    return projected
