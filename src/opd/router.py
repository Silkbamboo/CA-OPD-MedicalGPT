"""Constraint-aware teacher router (docs/METHOD.md §11.1 - §11.3).
...
final-test evaluator raises :class:`FinalTestLeakageError`; docs/METHOD.md §8.1
...
class RouterDecision:
    ...
    def as_metrics(self) -> Dict[str, float | str]:
        """Map onto the frozen metric vocabulary (docs/REPRODUCIBILITY.md §8)."""
        return {
...
class ConstraintAwareRouter:
    ...
    def _check_evaluator(evaluator: Optional[ControlEvaluator]) -> None:
        ...
        if not allows():
            ...
            raise FinalTestLeakageError(
                ...
                "(docs/METHOD.md §8.1: final test may not influence scheduling)"
            )
    ...
    def is_window_boundary(self, step: int) -> bool:
        ...
        Routing is updated per window, not per step (docs/METHOD.md §11.2:
        "初始建议按窗口更新").
        """
        if step <= 0:
            return False
...
class FixedRatioRouter:
    ...
    comes from, which is exactly what docs/METHOD.md §12 wants to ablate.
    """
