...
RUN_SCHEMA: Dict[str, object] = {
    "run": {
        ...
        "model": FieldSpec((str,)),
        "model_revision": FieldSpec((str,), doc="immutable 40-hex HF model snapshot"),
        "tokenizer_revision": FieldSpec((str,), doc="immutable tokenizer snapshot"),
        "seed": FieldSpec((int,), bounds=(0, None)),
        "output_root": FieldSpec((str,)),
        "configuration_status": FieldSpec(
            (str,),
            required=False,
            default="candidate_pending_gpu_calibration",
            choices=[
                "candidate_pending_gpu_calibration",
                "ready_pending_teacher_gate",
                "measured_on_target_hardware",
            ],
        ),
        "teacher_gate_config": FieldSpec((str,), required=False, default=None),
    },
    "data": {
        "protocol_version": FieldSpec((str,), choices=[DATA_PROTOCOL_VERSION]),
    ...
    "budget": {
        ...
        "price_per_gpu_hour_rmb": FieldSpec((float,), required=False, default=None, bounds=(0.0, None)),
        "price_status": FieldSpec((str,), choices=["candidate_pending_instance_page", "verified_instance_page"]),
        "hardware_calibration_status": FieldSpec((str,), choices=["candidate_pending_gpu_calibration", "measured_on_target_hardware"]),
    ...
    "verl": {
        ...
        "teacher_backend": FieldSpec(
            (str,), required=False, default="native", choices=["native", "shared_service"]
        ),
        "teacher_service_config": FieldSpec((str,), required=False, default=None),
    },
    "router": {
        "kind": FieldSpec((str,), choices=["constraint_aware", "fixed_ratio", "single_teacher", "sequential"]),
        ...
        "schedule": FieldSpec((list,), required=False, default=None),
    },
}
...
class LoadedRun:
    ...
    def as_dict(self) -> Dict[str, Any]:
        return {
            ...
            "model_provenance": {
                "model": self.raw["run"]["model"],
                "model_revision": self.raw["run"]["model_revision"],
                "tokenizer_revision": self.raw["run"]["tokenizer_revision"],
            },
            "data_protocol_version": self.raw["data"]["protocol_version"],
...
def load_run(config_path: str | Path) -> LoadedRun:
    ...
    for key in ("model_revision", "tokenizer_revision"):
        revision = str(run[key])
        if re.fullmatch(r"[0-9a-f]{40}", revision) is None:
            raise ConfigError(f"run.{key} must be an immutable 40-hex HF commit, got {revision!r}")
    ...
    plan = RunPlan(
        ...
        price_per_gpu_hour_rmb=(
            float(budget["price_per_gpu_hour_rmb"])
            if budget["price_per_gpu_hour_rmb"] is not None
            else None
        ),
    ...
    if router["kind"] == "sequential":
        schedule = router["schedule"]
        if not isinstance(schedule, list) or len(schedule) != 2:
            raise ConfigError("router.kind=sequential requires two schedule segments")
        expected_keys = {"teacher_route", "fraction"}
        if any(not isinstance(item, Mapping) or set(item) != expected_keys for item in schedule):
            raise ConfigError("sequential schedule entries require teacher_route and fraction")
        if [item["teacher_route"] for item in schedule] != ["medical", "base"]:
            raise ConfigError("SAR schedule must route medical then base")
        if abs(sum(float(item["fraction"]) for item in schedule) - 1.0) > 1e-9:
            raise ConfigError("sequential schedule fractions must sum to 1")
    if v["teacher_backend"] == "shared_service":
        if not v["teacher_service_config"]:
            raise ConfigError("teacher_backend=shared_service requires teacher_service_config")
        if len(verl.teachers) != 1 or verl.teacher_footprint() != 1:
            raise ConfigError("shared_service must reserve exactly one teacher GPU footprint")
    elif router["kind"] != "single_teacher" and len(verl.teachers) < 2:
        raise ConfigError(
