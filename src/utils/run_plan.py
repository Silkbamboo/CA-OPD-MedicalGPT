...
class ThroughputModel:
    ...
    deliberately conservative placeholders for a single RTX 4090 with a
    1.7B/4B LoRA setup; they must be replaced by measurements after the first
    smoke run.
    """
...
class RunPlan:
    ...
    num_gpus: int = 2
    price_per_gpu_hour_rmb: float = 2.5
    ...
    def __post_init__(self) -> None:
        ...
        if self.price_per_gpu_hour_rmb < 0:
    ...
    def estimate_cost_rmb(self) -> float:
    ...
    def as_dict(self) -> Dict[str, Any]:
        ...
        payload["derived"] = {
            ...
            **{k: round(v, 2) for k, v in self.estimate_seconds().items()},
            ...
            "estimated_cost_rmb": round(self.estimate_cost_rmb(), 2),
            "cost_cap_rmb": self.cost_cap_rmb,
            "within_cap": self.estimate_cost_rmb() <= self.cost_cap_rmb,
        }
    ...
    def to_markdown(self) -> str:
        ...
        lines = [
            ...
            f"| estimated cost | {d['estimated_cost_rmb']} RMB (cap {self.cost_cap_rmb}) |",
