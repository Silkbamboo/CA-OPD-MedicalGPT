...
#: C-Eval subjects that are medical and therefore must NOT act as general anchors.
...
class SourceSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if self.kind == "hf" and not self.hf_path:
...
def load_hf_source(spec: SourceSpec) -> List[StagedSample]:  # pragma: no cover - needs network
    """Load a Hugging Face dataset. Requires ``datasets`` and network access.

    Deliberately untested in CI: Phase 0 runs offline on a CPU box. The CLI
    surfaces the ImportError instead of silently falling back, so a missing
    dependency can never be mistaken for an empty dataset.
    """
    try:
    ...
    ds = load_dataset(spec.hf_path, spec.hf_name, split=spec.hf_split)
    return convert_records((dict(row) for row in ds), spec.converter, spec.max_samples)
