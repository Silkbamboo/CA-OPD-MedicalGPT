"""Chat template + loss-mask tests (agent.md §6.3: "chat template与loss mask快照测试").
...
def test_matches_real_tokenizer():
    ...
    Runs only when ``CA_OPD_MODEL_PATH`` points at local weights (i.e. on the
    training box after Qwen3-1.7B is downloaded). On the CPU dev box this is
    skipped rather than silently passing.
    """
    model_path = os.environ.get("CA_OPD_MODEL_PATH")
    if not model_path or not Path(model_path).exists():
        pytest.skip("CA_OPD_MODEL_PATH not set; real-tokenizer check deferred to the GPU box")
