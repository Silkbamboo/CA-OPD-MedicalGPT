"""OPD core mathematics tests (docs/REPRODUCIBILITY.md §6.1 / docs/REPRODUCIBILITY.md §6 "OPD核心").
...
def test_token_logprobs_matches_manual_gather_on_three_tokens():
    """Hand-checkable case: 1 sequence, 3 tokens, vocab 4 (docs/METHOD.md §14)."""
    logits = torch.tensor(
        [[
...
def test_student_and_teacher_score_identical_targets():
    """Teacher must not generate a second completion (docs/METHOD.md §9)."""
    batch = simple_batch()
...
def test_token_mean_has_no_length_bias_but_seq_mean_does():
    """docs/METHOD.md §9 / docs/REPRODUCIBILITY.md §6.1: reduction must not favour short sequences."""
...
# ---------------------------------------------------------------------------
# advantage clipping + domain-level KL safety scaling (docs/METHOD.md §11.4)
# ---------------------------------------------------------------------------
